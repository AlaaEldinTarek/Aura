...
/// notification centre while the app is running.
class DesktopNotificationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      // Use ignore when running as MSIX (shortcut already created by the installer)
      final isMsix = Platform.resolvedExecutable.contains('WindowsApps');
      await localNotifier.setup(
        appName: 'Aura | هالة',
        shortcutPolicy: isMsix ? ShortcutPolicy.ignore : ShortcutPolicy.requireCreate,
      );
      _initialized = true;
  ...
  /// Show an immediate notification (prayer time, achievement, etc.)
  Future<void> show({
  ...
  Future<void> showPostPrayerCheck(
    ...
    await show(
      ...
      body: isArabic ? 'افتح التطبيق لتسجيل صلاتك' : 'Open the app to log your prayer',
  ...
  Future<void> showAchievementNotification(
    ...
    final title = isArabic ? '$emoji إنجاز جديد!' : '$emoji Achievement Unlocked!';
  ...
  Future<void> showWirdReminder(int dailyGoal, bool isArabic, bool isJuzMode) async {
    final t = isArabic ? 'تذكير الورد' : 'Wird Reminder';
    final b = isArabic
        ? (isJuzMode ? 'اقرأ وردك اليومي' : 'اقرأ وردك اليومي — $dailyGoal صفحة')
        : (isJuzMode ? 'Read your daily Wird' : 'Read your daily Wird — $dailyGoal pages');
  ...
  /// Uses the same logic as NotificationService.scheduleTaskReminder.
  Future<void> scheduleTaskReminder({
    ...
    if (delay <= Duration.zero) {
      debugPrint('DesktopNotificationService: Task reminder time already passed for "$title"');
      return;
    }
    ...
    debugPrint('DesktopNotificationService: Task reminder "$title" in ${delay.inMinutes} min');
  }
  ...
  Future<void> scheduleWirdReminders({
    ...
    debugPrint('DesktopNotificationService: ${_wirdTimers.length} Wird timers set');
  }
  ...
  Future<void> scheduleDailyTaskDigest(int todayCount, int overdueCount) async {
    if (!isDesktop) return;
    _digestTimer?.cancel();
    _digestTimer = null;
    ...
    if (digestTime.isBefore(now)) return; // Already past 8 AM today
    ...
    debugPrint('DesktopNotificationService: Daily task digest timer set (in ${delay.inMinutes} min)');
  }

  Future<void> _showDailyTaskDigest(int todayCount, int overdueCount) async {
    final prefs = await SharedPreferences.getInstance();
    final isArabic = (prefs.getString('language') ?? 'en') == 'ar';
