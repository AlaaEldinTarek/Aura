...
| `PrayerWidgets.kt` | CombinedPrayerWidget (AllPrayersWidget) with ViewFlipper tabs (Next Prayer + Timeline), 4 layout variants. Colors: active gold `#F5B301`/`#B5821B`, bg from `widget_v3_bg_dark/light` (`#111317→#1A1B1E` / `#FFF8EB→#FFF3D6`). Root tap → Prayer tab (index 1) via `widget_target_tab=1` extra |
| `TasksWidget.kt` / `WidgetUpdateService.kt` / `DailyContentWidget.kt` | Home screen widgets. `TasksWidget` celebration state (all done): label turns gold (`#F5B301` dark / `#B5821B` light) — NOT green. Root tap → Tasks tab (index 3) via `widget_target_tab=3` extra. Task rows use `android:layoutDirection="ltr"` to prevent Arabic title text from breaking LTR row layout |
| `NativePrayerCalculator.kt` → `MainActivity.kt` | `handleWidgetTabIntent()` reads `widget_target_tab` from Intent extra (set by widgets), writes `widget_navigate_to_tab` to `${packageName}_preferences`. Flutter `_handleWidgetIntent()` reads this and calls `_handleTabTap(index)`. Works for both cold start (via `onCreate`) and warm start (via `onNewIntent`) |
...
- **Iqama time in prefs**: `PrayerAlarmReceiver` writes `adhan_iqama_time` (absolute ms timestamp) to `aura_prayer_times` when a prayer alarm fires. `AdhanFullScreenActivity` reads this to drive the post-adhan countdown. Value is 0 if iqama is not configured for that prayer. Per-prayer iqama keys (`fajr_iqama_time`, `dhuhr_iqama_time`, `asr_iqama_time`, `maghrib_iqama_time`, `isha_iqama_time`) are written to `aura_prayer_times` by Flutter's `prayerTimesProvider` side-effects alongside the regular `*_time` keys. `PrayerAlarmReceiver` reads these per-prayer keys to activate Adhan Mode in the foreground service notification (shows "Iqama in Xm Ys" countdown). Key for Zuhr is `dhuhr_iqama_time` (not `zuhr_iqama_time`).
- **`Task.isOverdue` day-level check**: For tasks WITHOUT a due time (`hasDueTime = false`), `isOverdue` compares due DATE vs today's date (not the exact timestamp). Without this, tasks due today always show as Overdue from the start of the day since `dueDate` is midnight. Tasks WITH a time use exact timestamp comparison. Never revert to a single `DateTime.now().isAfter(dueDate!)` — it breaks the Today section.
...
- **Tutorial system** (`lib/core/widgets/tutorial_overlay.dart`): `showTutorial(context, steps, onDone)` inserts an `OverlayEntry`. `TutorialStep` holds a `GlobalKey` (target), `titleKey`, `bodyKey`. `TutorialOverlay` uses `TickerProviderStateMixin` for two controllers: fade (300 ms, per-step) and pulse (1400 ms repeating). `_OverlayPainter` draws: ① dark overlay with `BlendMode.clear` cutout, ② solid border, ③ expanding+fading pulsing ring. Cutout radius is 20 (larger than card's 16 to avoid corner gaps). Per-screen tutorial keys live in `SharedPreferencesService` (`isTutorialXSeen()` / `setTutorialXSeen()`). Tutorial launches via `addPostFrameCallback` after 900 ms delay. **Do not add a tab-level Wird tutorial in `_onTabChanged`** — Wird content tutorial launches from `_WirdContentViewState.initState` instead. **Home tutorial launch**: uses `_pendingTutorialLaunch` bool flag (set by `_onPermissionsDone`) + `_launchTutorialWhenActive()` retry loop. The loop retries when: (a) a route is on top, (b) `mounted` is false, (c) something appears during the 800ms settle window. Never silently drops — always retries until tutorial launches or `isTutorialCompleted() == true`.
...
- **`dailyPrayerStatusProvider` after-midnight loading**: `load()` accepts an optional `fajrTime` parameter. When provided, it computes the effective date (between midnight and Fajr → yesterday) before querying Firestore. All callers that have access to `prayerTimesProvider` must pass fajr time. In `MainWrapperScreen.didChangeAppLifecycleState`, read fajr time **before** calling `ref.invalidate(prayerTimesProvider)` — invalidation clears the state, so reading after returns empty. The 30-second sync timer in `MainWrapperScreen` also passes fajr time.
- **`isPrayerTimeReached` guard**: `prayer_time_rules.dart` exports `isPrayerTimeReached(prayerName, prayerTimes)` — returns `true` only if `now >= prayer.time + 20 minutes` (same threshold as `canMarkPrayer`). Apply this filter anywhere a prayer status is DISPLAYED for today, to prevent showing future prayers as already done due to stale data or wrong-date Firestore records. Applied in: home screen (`build()` via `ref.watch`), prayer times screen (`_buildPrayerCards` render time + `_loadCompletedPrayers` + `ref.listen`), prayer tracking screen (`_buildDayDetails` for `isToday`). Past days are never filtered.
- **`_effectiveToday` in prayer tracking screen**: `_PrayerTrackingScreenState` stores `_effectiveToday` computed once in `initState` (same after-midnight logic as `_selectedDay`). The `ref.listen` on `dailyPrayerStatusProvider` uses `_effectiveToday` as the target key for `_monthlyData` — never recompute the target date inside the listener (prayerTimesProvider may not be loaded yet → race condition writes yesterday's data into today's slot). Guard: if `next.statuses.isEmpty` and `_monthlyData[_effectiveToday]` already has data, skip the write (prevents empty provider reload from clearing correctly-loaded calendar data).
- **Invalid prayer record cleanup**: `PrayerTrackingService.cleanupFuturePrayerRecords()` deletes today's Firestore records where `record.prayedAt < prayer.time + 20 min` — physically impossible for a legitimate user action (canMarkPrayer blocks early marking). Called once per day from `prayerTimesProvider` side-effects block (guarded by `_lastSideEffectsDate`). This removes records produced by bugs without touching legitimate data. Never delete based on current time alone — use `prayedAt` timestamp so the check is correct even if the cleanup runs after the prayer's window has passed.
